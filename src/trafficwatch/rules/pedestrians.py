...
FY_MIN_SPEED = 0.5           # vehicle speed through the crossing, body sizes / s
FY_REACH = 4.0               # pedestrian within this many vehicle sizes of the vehicle
RIDER_OVERLAP = 0.3
...
def _pedestrian_near(peds, crosswalk: str, veh: TrackData, sel: np.ndarray) -> bool:
    ...
    for name, ped, on in peds:
        ...
        if len(common) == 0:
            continue
        vpos = veh.pos[sel][iv]
        near = on[ip] & (np.linalg.norm(ped.pos[ip] - vpos, axis=1) < reach)
        if near.sum() >= 2:
            return True
    return False
