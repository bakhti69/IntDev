...
from ..tracks import TrackData, gaussian_smooth
...
# accident
ACC_CONTACT_DIST = 0.9
ACC_CONTACT_IOU = 0.05
ACC_PRE_SPEED = 0.8         # someone was moving before contact
ACC_DECEL = -1.5            # body sizes / s^2: impact-like deceleration
ACC_DEFLECT = np.deg2rad(30)
ACC_STOP_SPEED = 0.2
ACC_MAX_LEN = 10.0
# near_miss
NM_TTC = 1.5
...
def _accident_at(p: PairSeries) -> tuple[float, float] | None:
    ...
    for c in contact:
        ...
        if max(spd_a[pre].max(), spd_b[pre].max()) < ACC_PRE_SPEED or p.closing[pre].max() < 0.5:
            continue
        hit = _window(p.t, tc - 0.5, tc + 1.5)
        decel = min(p.a.accel[p.ia][hit].min(), p.b.accel[p.ib][hit].min())
        deflect = max(_heading_change(p.a, p.ia[hit]), _heading_change(p.b, p.ib[hit]))
        if decel > ACC_DECEL and deflect < ACC_DEFLECT:
            continue
        # after contact the road users stay together and come to rest (or leave)
        post = _window(p.t, tc, tc + 3.0)
        if np.median(p.dist[post]) > 1.5:
            continue
        both_stop = (spd_a < ACC_STOP_SPEED) & (spd_b < ACC_STOP_SPEED) & (p.t > tc)
        stops = np.where(both_stop)[0]
        end = p.t[stops[0]] if len(stops) else min(p.a.t[-1], p.b.t[-1])
...
def _evasive(tr: TrackData, idx: np.ndarray, t: np.ndarray) -> np.ndarray:
    """Per common sample: sharp braking from speed, or a swerve."""
    braking = (tr.accel[idx] <= NM_DECEL) & (np.maximum.accumulate(tr.speed[idx]) >= 0.8)
    swerve = (_heading_rate(tr, idx, t) >= NM_SWERVE_RATE) & (tr.speed[idx] >= 0.8)
